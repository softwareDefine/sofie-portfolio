...
const { login, logout, loginConfig, requireAdmin, requireInternal, resetRequest, resetConfirm } = require('../auth');
...
router.get('/projects/:id(\\d+)', async (req, res, next) => {
    try {
        ...
        if (!rows.length) return res.status(404).json({ error: '없는 프로젝트' });
        res.json(rows[0]);
    } catch (e) { next(e); }
});
